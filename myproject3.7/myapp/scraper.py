...
# bilibili 和 youtube长视频属于类型1
def extract_type1(output):
    pattern = r'Merging formats into "(.+?)"'
...
# tiktok 和 souhu,youtube短视频属于类型2
def extract_type2(output):
    # 定义正则表达式来匹配文件路径部分
...
def download_video(url, website_type):
    # 获取当前脚本文件的绝对路径
    ...
    try:
        ...
        print(output)
        if website_type == 1:
            output_filename = extract_type1(output)
        else:
            output_filename = extract_type2(output)
